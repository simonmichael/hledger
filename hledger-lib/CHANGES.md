...
- fix:lib:invertAmount: with zero amounts, do nothing instead of failing [#2476]
...
- fix:add,import: read all -f files, not just the first [#2553]
  This fixes a regression in 1.51.2.
  withPossibleJournal was only reading the first -f file (NE.head),
  discarding the rest. This broke autocompletion in `add` and multi-file
  reading in `import` when multiple -f flags were given.

  Add readPossibleJournalFiles (plural) which reads the first file
  tolerantly (may not exist) and the rest normally, then combines them.
...
- imp:cli: when less options are failing, fall back to unpaged output (and a warning)

- dev:cli: handle a number of runPager failure modes more robustly

- dev: properly convert {{LOTTOTALCOST}} to a unit cost internally
...
- fix: postings generated by --infer-equity no longer inherit tags/comment [#2535]
  Any commment, tags, and real/virtual/balanced virtual type of the cost
  posting was being propagated to the new equity postings; now they are
  not. (posting date and posting status still are.)

- ;dev: Hledger.Data.Posting: syntactic change to help VSC highlighter

- dev: fix a warning introduced today [#2528]
...
- Revert "fix:lib:Hledger.Query: words'': Parse escaped quotes in quoted patterns" [#2513]
  This reverts commit de900e5b24fa4fbd4c6aa17814e6711f153842c5, it's not
  as safe as we thought.

- imp:add,import: also create directories when autocreating journal file
...
- imp:lib:valuation: optimize price lookup with pre-built indexes [#2511] (Oleg Bulatov)
...
- fix:lib:Hledger.Query: words'': Parse escaped quotes in quoted patterns (Tuong Nguyen Manh)
