...
- ;tools: just: note newer bash required

- ;pkg:stack910: bump to latest snapshot

- ;doc:REGRESSIONS

- ;examples:csv:fidelity: edits

- ;examples:csv:fidelity updates

- ;doc:ACHIEVEMENTS

- ;bin: smooth: accept ACCTPAT, matching case-insensitively as infix (like aregister)

- fix: fancyassertions: Nicer show call (Joshua Chapman)
  - As part of its output fancyassertions writes
    relevant balances: and then the balances that caused
    the failure
  - Now it uses the showMixedAmount function instead to
    properly format the amount as expected by users

- ;doc:REGRESSIONS: fix issue number
