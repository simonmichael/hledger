...
aregister opts@CliOpts{rawopts_=rawopts,reportspec_=rspec} j = do
  ...
  let
    ...
    render | fmt=="txt"  = accountTransactionsReportAsText opts (_rsQuery rspec') thisacctq matching
           ...
           | fmt=="csv"  = printCSV . accountTransactionsReportAsCsv hd wd (_rsQuery rspec') thisacctq matching
           | fmt=="tsv"  = printTSV . accountTransactionsReportAsCsv hd wd (_rsQuery rspec') thisacctq matching
           | fmt=="fods" =
                printFods IO.localeEncoding . Map.singleton "Aregister" .
                (,) (1,0) .
                accountTransactionsReportAsSpreadsheet oneLineNoCostFmt hd wd (_rsQuery rspec') thisacctq matching
           | fmt=="json" = toJsonText
...
accountTransactionsReportAsCsv ::
  Bool -> WhichDate -> Query -> Query -> Bool -> AccountTransactionsReport -> CSV
accountTransactionsReportAsCsv hd wd reportq thisacctq matching =
  Spr.rawTableContent .
  accountTransactionsReportAsSpreadsheet machineFmt hd wd reportq thisacctq matching

accountTransactionsReportAsSpreadsheet ::
  AmountFormat -> Bool ->
  WhichDate -> Query -> Query -> Bool -> AccountTransactionsReport ->
  [[Spr.Cell Spr.NumLines Text]]
accountTransactionsReportAsSpreadsheet fmt hd wd reportq thisacctq matching is =
  optional hd
    [Spr.addHeaderBorders $ map Spr.headerCell $
      ["txnidx","date","code","description","otheraccounts","change","balance"]]
  ++
  map (accountTransactionsReportItemAsRecord fmt True wd reportq thisacctq matching) is
...
accountTransactionsReportItemAsRecord ::
  ...
  [Spr.Cell Spr.NumLines Text]
accountTransactionsReportItemAsRecord
  fmt internals wd reportq thisacctq matching
  ...
  = (optional internals [Spr.integerCell tindex]) ++
    ...
    [cell tdescription,
     cell $ if matching then matchedAcct else T.intercalate ", " $ nub otheraccts,
     amountCell change,
     amountCell balance]
  where
...
accountTransactionsReportItemAsText
  ...
  where
    ...
    accts = if matching 
            then T.intercalate ", " . map accountSummarisedName $ nub matchedaccts
            else T.intercalate ", " . map accountSummarisedName $ nub otheraccts
      where
