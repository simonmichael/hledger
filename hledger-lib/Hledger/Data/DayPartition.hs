...
-- | A partition of time into spans of days and a historical period.
...
spansFromBoundaries e xs     = boundariesToMaybeDayPartition $ takeUntilFails (<e) xs
  where
    takeUntilFails _ []     = []
    takeUntilFails p (y:ys) = if not (p y) then [y] else y : takeUntilFails p ys
