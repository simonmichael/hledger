...
| **[`commodity`]**             | Declares <br>1. a commodity symbol, for checking all amounts in all files <br>2. the commodity's display style <br>3. (if there is no `decimal-mark` directive:) the decimal mark for parsing this commodity, in the rest of this file and subfiles. <br> Takes precedence over `D`. <br>Subdirectives: `format` (ignored). <br>Command line equivalent: [`-c/--commodity-style`](#commodity-styles)          | N,<br>N,<br>Y |
