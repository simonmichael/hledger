...
getAmount rules record currency p1IsVirtual n =
  let
    ...
    fieldnames = map (("amount"++show n)++) ["","-in","-out"]
                 -- For posting 1, also recognise the old field names.
    ...
    nonzeroamounts = [(f,a') | f <- fieldnames
                     ...
                     , not $ isZeroMixedAmount a
                       -- With an old field name, flip the sign in posting 2
                     , let a' = if f `elem` unnumberedfieldnames && n==2 then -a else a
                     ]
