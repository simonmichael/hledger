...
sortAccountTreeByAmount normalsign = mapAccounts (\a -> a{asubs=sortSubs $ asubs a})
  where
    sortSubs subs = case normalsign of
        NormallyPositive -> sortOn (Down . amt) posSubs ++ sortOn amt negSubs
        NormallyNegative -> sortOn amt negSubs ++ sortOn (Down . amt) posSubs
      where
        (negSubs, posSubs) = partition (fromMaybe False . isNegativeMixedAmount . aibalance) subs
