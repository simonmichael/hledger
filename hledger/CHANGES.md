...
- ;doc:P directive: fix typo

- ;doc:csv:if:field matchers: clarify

- ;doc:journal:code: mention valid characters, recommend tags  [#2563]

- ;doc: aregister: Add new --drop flag (Caleb Maclennan)

- test: Add unit test for now --drop frog on aregister (Caleb Maclennan)

- feat: aregister: Implement --drop flag (Caleb Maclennan)
  This implements the same --drop functionality as balance and register
  commands have for the aregister subcommand, allowing users to trim
  leading account name components from register output.

- dev:ui: use divideSafe in a few more places [#2476]

- fix:run: when running commands from stdin, don't hide errors [#2557]
...
- fix:cli: fix options requiring a value consuming unrelated arguments [#2556]
...
- fix:balance: fix --budget CSV/TSV output using thousands separators [#2555]
...
- ;test: commit dummy addon scripts so addons.test works in fresh clones/worktrees
  Previously these were generated by `just mktestaddons` and gitignored,
  so addons.test would fail in any fresh checkout. Now they are committed
  directly and the generator recipe is removed.
