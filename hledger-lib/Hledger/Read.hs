...
-- If the specified file does not exist, an error is raised.
--
-- If LEDGER_FILE is unset or set to the empty string, we return a default file path:
-- @.hledger.journal@ in the user's home directory (or if we can't find the user's home directory,
...
defaultJournalPath = do
  ...
  then do
    ...
    case mf of
      Just f -> return f
      Nothing -> error' $ "LEDGER_FILE " <> glob <> " was not found"
