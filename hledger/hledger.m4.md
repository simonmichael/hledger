...
To ignore a commodity in all reports by default, try adding `not:cur:commodity-name` in your [config file](#config-files).
...
. Note that omitting the commodity symbol will set the display style for just the 
